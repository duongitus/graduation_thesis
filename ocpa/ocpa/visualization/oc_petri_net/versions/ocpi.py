...
def apply(obj, parameters=None):
    ...
    for tr in obj.transitions:
        ...
        if tr.silent == True:
            ...
            #       labelfontsize="36.0")
            g.node(tr.name, tr.label, shape="box", fontsize="36.0",
                  labelfontsize="36.0")
    ...
    for arc in obj.arcs:
        ...
        if arc.variable == True:
            g.edge(all_objs[source_node], all_objs[target_node],
                   label="", color=color + ":white:" + color, fontsize="13.0", arrowhead = "None")
        else:
            g.edge(all_objs[source_node], all_objs[target_node],
                   label="", color=color, fontsize="13.0", arrowhead = "None")
