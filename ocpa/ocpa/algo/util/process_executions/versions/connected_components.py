...
def apply(ocel, parameters):
    ...
    log_df["event_objects"] = log_df.apply(
        lambda x: set([(ot, o) for ot in ocel.object_types for o in x[ot]]), axis=1
    )
    cases = sorted(
        nx.weakly_connected_components(ocel.graph.eog), key=len, reverse=True
    )
    obs = []
    mapping_objects = dict(zip(log_df["event_id"], log_df["event_objects"]))
