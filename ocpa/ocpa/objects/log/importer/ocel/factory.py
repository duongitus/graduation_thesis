...
VERSIONS = {OCEL_JSON: import_ocel_json.apply, OCEL_XML: import_ocel_xml.apply}


def apply(
    file_path, variant=OCEL_JSON, parameters=None, file_path_object_attribute_table=None
) -> OCEL:
    """
    ...
    parameters: dict
        parameters that will be used for importing the log and for log settings:
            - execution_extraction: Optional, execution extraction technique to extract process executions (cases) in the log, possible values:
                ...
                - :data:`ocpa.algo.util.process_executions.factory.LEAD_TYPE <ocpa.algo.util.process_executions.factory.LEAD_TYPE>`
