...
@dataclass
class Event:
    __slots__ = ("id", "act", "time", "omap", "vmap")
    id: str
    act: str
    time: datetime
    omap: list[str]
...
class RawObjectCentricData:
    __slots__ = ("events", "objects", "obj_event_mapping")
...
class ObjectCentricEventLog:
    ...
    def __post_init__(self):
        ...
        self.activities = set([self.raw.events[e].act for e in self.raw.events])
        self.act_events = {}
    ...
    def eve_ot_objects(self, eid: str, ot: str) -> list[str]:
        return [
            oid for oid in self.eve_objects[eid] if self.raw.objects[oid].type == ot
        ]
    ...
    def coexistence(self, ot: str, act1: str, act2: str):
        return [
            ...
            if (act1 not in self.trace[o] and act2 not in self.trace[o])
            or (act1 in self.trace[o] and act2 in self.trace[o])
        ]
        ...
        for eid in self.act_events[act]:
            if (
                len(
                    [
                        oid
                        for oid in self.eve_objects[eid]
                        if self.raw.objects[oid].type == ot
                    ]
                )
                == 0
            ):
    ...
    def num_ot_objects_containing_act1_followed_by_act2(
        self, ot: str, act1: str, act2: str
    ) -> int:
        objects = []
        for oid in self.ot_objects[ot]:
            trace = self.trace[oid]
    ...
    def causal_relation(self, ot: str, act1: str, act2: str):
        ...
        if num_acts > 0:
            return (
                self.num_ot_objects_containing_act1_followed_by_act2(ot, act1, act2)
                / num_acts
            )
        else:
            return 0
    ...
    def choice_relation(self, ot: str, act1: str, act2: str):
        num_act1 = self.num_ot_objects_containing_acts(ot, [act1])
        num_act2 = self.num_ot_objects_containing_acts(ot, [act2])
        sum_num = num_act1 + num_act2
        print(num_act1, num_act2, sum_num)
        if sum_num > 0:
            num_act1_and_act2 = self.num_ot_objects_containing_acts(ot, [act1, act2])
